...
//    }
//以下为修改
		if(0x01 == issuedData->attrFlags.flaggps_jingdu)
    {
        info->event[info->num] = EVENT_gps_jingdu;
        info->num++;
        dataPoints->valuegps_jingdu = gizX2Y(gps_jingdu_RATIO,  gps_jingdu_ADDITION, exchangeWord(issuedData->attrVals.valuegps_jingdu));
    }
        
    if(0x01 == issuedData->attrFlags.flaggps_weidu)
    {
        info->event[info->num] = EVENT_gps_weidu;
        info->num++;
        dataPoints->valuegps_weidu = gizX2Y(gps_weidu_RATIO,  gps_weidu_ADDITION, exchangeWord(issuedData->attrVals.valuegps_weidu));
    }
...
//以下为修改
		if(last->valuegps_jingdu != cur->valuegps_jingdu)
    {
        GIZWITS_LOG("valuegps_jingdu Changed\n");
        ret = 1;
    }
    if(last->valuegps_weidu != cur->valuegps_weidu)
    {
        GIZWITS_LOG("valuegps_weidu Changed\n");
        ret = 1;
    }

    if(last->valuegps_speed != cur->valuegps_speed)
    {
        if(gizGetTimerCount()-lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuegps_speed Changed\n");
...
//		以下为修改
		devStatusPtr->valuegps_jingdu = exchangeWord(gizY2X(gps_jingdu_RATIO,  gps_jingdu_ADDITION, dataPoints->valuegps_jingdu)); 
    devStatusPtr->valuegps_weidu = exchangeWord(gizY2X(gps_weidu_RATIO,  gps_weidu_ADDITION, dataPoints->valuegps_weidu)); 
    devStatusPtr->valuegps_speed = exchangeWord(gizY2X(gps_speed_RATIO,  gps_speed_ADDITION, dataPoints->valuegps_speed)); 
    return 0;
}
